...
//        // 从Session中获取保存的验证码
//        String codeFromUser = (String) session.getAttribute(phone);

        // 从Redis中获取保存的验证码
        String codeFromUser = redisTemplate.opsForValue().get(phone);

        // 将页面提交的和Session中保存的验证码进行比对
        if (!StringUtils.equals(code, codeFromUser)) {
        ...
        // 如果用户登陆成功, 就可以删除Redis中对于该用户的缓存验证码
